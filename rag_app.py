...
Uses Google Gemini's FREE API (no billing required for the free tier).
...
def create_embedding(text, task_type="retrieval_document"):
    """
    Create an embedding (numerical representation) of text using Gemini.
...
def find_relevant_chunks(question, chunks_with_embeddings, top_k=3):
    ...
    """
    # Get embedding for the question (use query task type for better results)
    question_embedding = create_embedding(question, task_type="retrieval_query")
...
def generate_answer(question, context_chunks):
    """
    Generate an answer using Gemini based on relevant context.
